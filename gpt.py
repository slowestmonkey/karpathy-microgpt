...
def gpt(token_id, pos_id, keys, values):
    ...
    x = rmsnorm(x) # Do we really need it?

    for layer_index in range(n_layer):
        # 1) Multi-head attention block
        x_residual = x
        x = rmsnorm(x)
        q = linear(x, state_dict[f'layer{layer_index}.attn_wq']) # Iterate on what's happening in linear and why we need it
        ...
        for h in range(n_head):
            ...
            k_h = [ki[hs:hs+head_dim] for ki in keys[layer_index]]
            v_h = [vi[hs:hs+head_dim] for vi in values[layer_index]]
